# resolucao-cartao Specification

## Purpose
Fornecer uma resolução centralizada e unificada para cartões de crédito e formas de pagamento nos lançamentos e despesas do Divi, eliminando duplicações de código no backend.
...
- **WHEN** o método de pagamento é `card` e o `cardOwnerId` corresponds ao `id` de um cartão existente
...
- **WHEN** o método de pagamento é `card` e o `cardOwnerId` corresponds ao `responsavelPadraoId` de um cartão existente
- **THEN** a função SHALL retornar o `id` do cartão como `cartaoId`, o `responsavelPadraoId` do cartão como `cardOwner`, e o `responsavelPadraoId` como `responsavelFaturaId`
