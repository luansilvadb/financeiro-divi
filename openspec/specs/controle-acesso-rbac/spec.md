...
O sistema DEVE permitir a definição de papéis de acesso (ADMIN, MORADOR, VISUALIZADOR) para cada morador da casa, determinando quais ações ele pode executar no sistema de forma transparente.
...
- **THEN** o sistema atualiza o cargo do morador para ADMIN no banco de dados e propaga a alteração no frontend em tempo real
...
- **THEN** o sistema rejeita a operação com erro HTTP 403 (Forbidden)
...
- **THEN** o sistema bloqueia a alteração no frontend e backend, exibindo uma mensagem de erro apropriada
