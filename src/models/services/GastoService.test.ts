...
describe('GastoService.atualizarGastoCompleto', () => {
  ...
  it('atualiza um gasto individual preservando seus metadados', async () => {
    ...
    const atualizado = vi.mocked(gastoRepo.atualizar).mock.calls[0][1]
    expect(atualizado.faturaId).toBe('f2')
    expect(atualizado.descricao).toBe('Atualizado')
    expect(atualizado.recurringBillId).toBe('conta-1')
    expect(atualizado.cardOwner).toBe('m1')
    expect(atualizado.isPrivate).toBe(true)
    expect(atualizado.splitMode).toBe('income')
  })
  ...
  it('atualiza todas as parcelas sem recriar o grupo quando sua estrutura nao muda', async () => {
    ...
    const chamadas = vi.mocked(gastoRepo.atualizar).mock.calls
    expect(chamadas).toHaveLength(2)
    expect(chamadas.map(c => c[1].installments)).toEqual([2, 1])
