...
describe('FaturaService', () => {
  ...
  it('deve assegurar faturas abertas para cartoes cadastrados', async () => {
    ...
    const faturaRepo = {
      listarTodas: vi.fn()
        .mockResolvedValueOnce(faturasExistentes)   // first call during check
        .mockResolvedValueOnce(faturasExistentes),   // second call after batch (re-fetch for real UUIDs)
      salvarMuitas: vi.fn()
    }
    const service = new FaturaService(faturaRepo as any)
    ...
    expect(faturaRepo.listarTodas).toHaveBeenCalledTimes(2)
