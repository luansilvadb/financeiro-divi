...
<template>
  <div class="max-w-6xl mx-auto p-4 md:p-6 space-y-6">
    <!-- Top Header -->
    <div class="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm flex flex-col md:flex-row md:justify-between md:items-center gap-4">
      <div class="flex items-center gap-4">
        <button 
          @click="emit('voltar')"
          class="w-10 h-10 rounded-full bg-slate-50 hover:bg-slate-100 text-gray-500 font-bold flex items-center justify-center border border-slate-100 transition-all active:scale-95"
        >
          ➔
        </button>
        <div>
          <h2 class="text-xl font-black text-gray-800 flex items-center gap-2">
            Revisão da Fatura
            <span class="text-xs bg-amber-100 text-amber-800 font-black px-2 py-0.5 rounded-full uppercase tracking-wider">Fechada</span>
          </h2>
      ...
      <div class="flex gap-2">
        <button 
          @click="reabrirFatura"
          class="px-4 py-2.5 border-2 border-rose-100 hover:border-rose-200 text-rose-600 rounded-2xl text-xs font-black transition-all active:scale-95 bg-rose-50/20"
        >
          🔓 Reabrir Fatura
        </button>
        <button 
          ...
          class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 text-white rounded-2xl text-xs font-black shadow-md hover:shadow-lg transition-all active:scale-95 flex items-center gap-2"
        >
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div class="lg:col-span-7 bg-white border border-slate-100 rounded-3xl p-5 md:p-6 shadow-sm space-y-4">
        <ListaGastosRevisao 
          :gastos="gastosFatura" 
