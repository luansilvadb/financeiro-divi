...
export const formatarCentavosParaBRL = (centavos: number | bigint, incluirSimbolo: boolean = true): string => {
  ...
  if (!Number.isSafeInteger(num)) {
    // For values beyond safe integer range (astronomically large for household use),
    // fall back to displaying as-is with 2 decimal places.
    const str = typeof centavos === 'bigint' ? centavos.toString() : String(centavos)
    ...
    return incluirSimbolo ? `R$ ${formatted}` : formatted
  }
  return formatarBRL(num / 100, incluirSimbolo)
}
