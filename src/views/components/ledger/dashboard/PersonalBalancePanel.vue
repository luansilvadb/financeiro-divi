...
const confirmarLiquidar = async (dados: { from: string; to: string; valor: number; method: 'pix' | 'cash'; descricao: string }) => {
  ...
  try {
    ...
    await gastoService.lancarGastoOuEmprestimo(data as any)
...
const handleExcluirGasto = async (g: Gasto) => {
  if (confirm('Deseja realmente estornar este lançamento privado?')) {
    try {
